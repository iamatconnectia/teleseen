...
class FirebaseServices extends BaseFirebaseServices {
  ...
  Future<void> init() async {
    ...
    await FirebaseServiceFactory.create<FirebaseCoreService>()?.initializeApp();
  ...
  Future<void> signOut() async {
    if (isEnabled) {
      ...
      try {
        ...
        printLog('[FirebaseServices] Error deleting messaging token: $e');
      }
    }
  }
  ...
  Future<void> loginFirebaseGoogle({String? idToken, String? accessToken}) async {
    if (FirebaseServices().isEnabled) {
      await _auth?.loginFirebaseGoogle(idToken: idToken, accessToken: accessToken);
    }
  }
  ...
  @override
  dynamic getCurrentUser() => _auth?.currentUser;
