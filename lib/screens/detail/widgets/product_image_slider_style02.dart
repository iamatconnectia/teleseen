...
class _ProductImageSliderStyle02State extends State<ProductImageSliderStyle02>
  ...
  List<String> get itemList {
    ...
    if (list.isEmpty ||
        (list.length == 1 &&
            (list[0].contains('no_product_image') ||
                list[0].contains('placeholder')))) {
      final fitnessPlaceholder =
          ImageTools.getFitnessPlaceholder(widget.product.name);
      if (fitnessPlaceholder != null) {
        return [fitnessPlaceholder];
      }
    } else {
      /// Check if the list contains the placeholder and replace it with the fitness placeholder
      for (var i = 0; i < list.length; i++) {
        if (list[i].contains('no_product_image') ||
            list[i].contains('placeholder')) {
          final fitnessPlaceholder =
              ImageTools.getFitnessPlaceholder(widget.product.name);
          if (fitnessPlaceholder != null) {
            list[i] = fitnessPlaceholder;
          }
        }
      }
    }
  ...
  List<String> get imagesList {
    var list = {..._images, ..._variationImages}.toList();
    if (list.isEmpty ||
        (list.length == 1 &&
            (list[0].contains('no_product_image') ||
                list[0].contains('placeholder')))) {
