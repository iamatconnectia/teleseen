...
class _StateChooseAddress extends BaseScreen<ChooseAddressScreen> {
  ...
  Widget _renderBillingAddress() {
    ...
    return GestureDetector(
      onTap: () {
        final add = Address(
            firstName: (billing.firstName?.isNotEmpty ?? false)
                ? billing.firstName
                : user?.firstName,
            lastName: (billing.lastName?.isNotEmpty ?? false)
                ? billing.lastName
                : user?.lastName,
            email: (billing.email?.isNotEmpty ?? false)
                ? billing.email
                : user?.email,
  ...
  Widget _renderShippingAddress() {
    final shipping = user?.shipping;
    final billing = user?.billing;
    if (user == null || shipping == null) return const SizedBox();
    final userFullName =
        '${billing?.firstName ?? ''} ${billing?.lastName ?? ''}'
            .trim();
    return GestureDetector(
      onTap: () {
        final add = Address(
          firstName: (shipping.firstName?.isNotEmpty ?? false)
              ? shipping.firstName
              : user?.firstName,
          lastName: (shipping.lastName?.isNotEmpty ?? false)
              ? shipping.lastName
              : user?.lastName,
          email: user?.email,
