...
extension on _ShippingAddressState {
  ...
  void loadUserInfoFromAddress(Address? address) {
    ...
    _textControllers[AddressFieldType.email]?.text =
        address?.email?.trim() ?? '';
    _textControllers[AddressFieldType.deliveryArea]?.text =
        address?.deliveryArea?.trim() ?? _selectedDeliveryArea ?? '';
    _selectedDeliveryArea = address?.deliveryArea ?? _selectedDeliveryArea;
  }
  ...
  Future<void> _onNext() async {
    if (_isLoadingShipping) {
      // Shipping cost is still being calculated - show a brief message and wait
      FlashHelper.errorMessage(
        context,
        message: 'Calculating shipping cost, please wait...',
      );
      return;
    }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      // Set the selected delivery area in the cart model address
      if (_selectedDeliveryArea != null && address != null) {
        address!.deliveryArea = _selectedDeliveryArea;
      }

      final cartModel = Provider.of<CartModel>(context, listen: false);
      cartModel.setAddress(address);
      
      print("Final Applied Shipping Cost: ${cartModel.shippingCost}");
      print("Subtotal: ${cartModel.getSubTotal()}");
      print("Final Total: ${cartModel.getTotal()}");
  ...
  void onTextFieldSaved(String? value, AddressFieldType type) {
    switch (type) {
      ...
      case AddressFieldType.zipCode:
        ...
        break;
      case AddressFieldType.deliveryArea:
        _selectedDeliveryArea = value;
        if (address != null) {
          address!.deliveryArea = value;
        }
        break;
  ...
  Widget _buildBottom(bool isDesktop) {
    final textButton = _isLoadingShipping
    ...
    return CheckoutActionWidget(
      ...
      iconPrimary: _isLoadingShipping ? Icons.hourglass_top : Icons.local_shipping_outlined,
