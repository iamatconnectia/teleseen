...
import '../../../models/index.dart'
    show AppModel, CartModel, PaymentMethod, PaymentMethodModel, TaxModel;
...
class _PaymentMethodsState extends State<PaymentMethods>
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Consumer2<PaymentMethodModel, WalletModel>(
              ...
              var paymentMethods =
                  List<PaymentMethod>.from(model.paymentMethods);

              // Remove any COD methods
              paymentMethods.removeWhere((p) =>
                  p.id == 'cod' ||
                  p.id == 'cashondelivery' ||
                  (p.title?.toLowerCase().contains('cash on delivery') ??
                      false));

              // Force inject KNET if not present
              if (!paymentMethods.any((p) =>
                  p.id!.toLowerCase().contains('knet') ||
                  (p.title?.toLowerCase().contains('knet') ?? false))) {
                paymentMethods.add(PaymentMethod(
                  id: 'knet',
                  title: 'KNET',
                  description: 'Pay securely with KNET.',
                  enabled: true,
                ));
              }

              if (paymentMethods.isEmpty && model.message != null) {
                return SizedBox(
                  height: 100,
                  child: Center(
              ...
              final isWalletExisted = paymentMethods
              ...
              if (selectedId == null && paymentMethods.isNotEmpty) {
                ...
                final selectedMethod = paymentMethods.firstWhereOrNull(
                  (item) => item.id == selectedId,
                );
                if (selectedMethod != null) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    cartModel.setPaymentMethod(selectedMethod);
                  });
                }
              }
  ...
  Widget _buildBottom(PaymentMethodModel paymentMethodModel, cartModel) {
    return CheckoutActionWidget(
      ...
      onTapPrimary: () => isPaying || selectedId == null
              ? showSnackbar()
