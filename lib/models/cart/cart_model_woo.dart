...
class CartModelWoo
  ...
  Future<void> initData() async {
    ...
    await getAddress();
    getCurrency();
    // Restore persisted shipping cost (set by the custom Delivery Area flow)
    final savedCost = UserBox().savedShippingCost;
    if (savedCost > 0) {
...
// Removes everything from the cart.
  ...
  @override
  Future<void> setShippingMethod(data) async {
    await super.setShippingMethod(data);
    scheduleMicrotask(notifyListeners);
  }

  @override
  void setPaymentMethod(data) {
    super.setPaymentMethod(data);
    scheduleMicrotask(notifyListeners);
  }

  @override
  void setShippingCost(double cost) {
    super.setShippingCost(cost);
    scheduleMicrotask(notifyListeners);
  }
  ...
  double? getShippingCost({bool includeTax = false}) {
    ...
    if (isMultiVendor) {
      final vendorCost = getShippingVendorCost(includeTax: includeTax);
      if (vendorCost > 0 || selectedShippingMethods.isNotEmpty) {
        return vendorCost;
      }
    }
    return super.getShippingCost(includeTax: includeTax);
  }
