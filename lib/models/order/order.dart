...
class Order {
  ...
  Map<String, dynamic> toJson(CartModel cartModel, userId, paid) {
    ...
    var params = {
      ...
      'customer_id': int.tryParse(userId?.toString() ?? '') ?? 0,
    ...
    try {
      ...
      if (isMultiVendor) {
        ...
        if (kPaymentConfig.enableShipping && (cartModel.shippingMethod != null || cartModel.shippingCost > 0)) {
          final shippingFee = PriceTools.getPriceValueByCurrency(
          ...
          params['shipping_lines'] = [
            {
              'method_id': cartModel.shippingMethod?.id ?? 'flat_rate',
              'method_title': cartModel.shippingMethod?.title ?? 'Flat Rate',
  ...
  Order._fromBigCommerceJson(Map parsedJson) {
    try {
      id = parsedJson['id']?.toString();
      customerId = parsedJson['customer_id']?.toString();
      // Parse date time RFC-2822
      try {
        if (parsedJson['date_created'] != null) {
          createdAt = DateFormat("EEE, d MMM yyyy HH:mm:ss Z")
              .parse(parsedJson['date_created']);
        } else {
          createdAt = DateTime.now();
        }
      } catch (e) {
        createdAt = DateTime.tryParse(parsedJson['date_created'] ?? '') ??
            DateTime.now();
      }

      try {
        if (parsedJson['date_modified'] != null) {
          dateModified = DateFormat("EEE, d MMM yyyy HH:mm:ss Z")
              .parse(parsedJson['date_modified']);
        } else {
          dateModified = DateTime.now();
        }
      } catch (e) {
        dateModified = DateTime.tryParse(parsedJson['date_modified'] ?? '') ??
            DateTime.now();
      }
      totalTax = parsedJson['total_tax'] != null
          ? double.parse(parsedJson['total_tax'])
  ...
  @override
  String toString() =>
      'Order {id: $id, status: $status, paymentMethod: $paymentMethod, paymentUrl: $paymentUrl}';
}
