...
class WooWidget extends BaseFrameworks
  ...
  void placeOrder(context,
    ...
    final paymentId = paymentMethod?.id?.toLowerCase();
    ...
    if (paymentId == 'knet') {
      createOrder(context,
          ...
          success: (Order order) async {
            ...
            onLoading?.call(true);
            var knetPaymentUrl = await (Services().api as WooCommerceService)
                .getKnetPaymentUrl(order.id);
            onLoading?.call(false);
            
            var paymentUrl = knetPaymentUrl?.isNotEmpty == true ? knetPaymentUrl : order.paymentUrl;
            print('KNET CUSTOM API URL: $knetPaymentUrl');
            print('FINAL PAYMENT URL TO OPEN: $paymentUrl');

            if (paymentUrl?.isNotEmpty ?? false) {
              printLog('[placeOrder] KNET URL recognized: $paymentUrl');
              printLog('[placeOrder] Pushing PaymentWebview...');
              await Navigator.push(
                context,
                MaterialPageRoute(
              ...
              print('ERROR: KNET Payment URL is missing.');
              Tools.showSnackBar(
                  ScaffoldMessenger.of(context), 'Payment URL not received from server.');
              // Do NOT call success!(order) here to prevent success screen showing
            }
          },
          error: error);
      return;
