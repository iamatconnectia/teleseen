    ...
    try {
      ...
      if (res is Map && isNotBlank(res['message'])) {
        ...
        await Future.forEach(res, (zone) async {
          ...
          print("FETCHING METHODS FOR ZONE ID: $zoneId");
          var response = await wcApi.getAsync('shipping/zones/$zoneId/methods');
          print("Shipping API Response for Zone $zoneId: $response");
          
          if (response is Map && isNotBlank(response['message'])) {
            throw Exception(response['message']);
          } else if (response is List) {
            for (var item in response) {
              final cost = item['settings']?['cost']?['value'] ?? '0';
              print("Parsed Method ${item['method_id']} Cost: $cost");
              var methodItem = {
                'id': item['instance_id'],
      ...
      if (list.isEmpty) {
        // Return empty list instead of throwing to allow frontend to handle gracefully
        return list;
      }
      return list;
    } catch (err) {
...
//          }
  ...
  Future<Order?> createOrder({
    ...
    try {
      ...
      final response = await wcApi.postAsync('orders', params, version: 3);
      if (response is Map && response['id'] != null) {
        return Order.fromJson(Map<String, dynamic>.from(response));
      } else {
        throw Exception(response['message'] ?? 'Error creating order');
      }
  ...
  Future<String?> getKnetPaymentUrl(String? orderId) async {
    try {
      if (orderId == null) return null;
      final response = await wcApi.postAsync(
        '../teleseen/v1/knet/payment-url',
        {'order_id': orderId},
      );
      if (response is Map && response['payment_url'] != null) {
        return response['payment_url'];
      }
    } catch (e) {
      printLog('[getKnetPaymentUrl] error: $e');
    }
    return null;
  }
  ...
  Future<Map<String, dynamic>?> updateUserInfo(
    ...
    try {
      ...
      printLog('Update User URL: $domain/wp-json/api/flutter_user/update_user_profile');
      printLog('Update User Cookie: $token');
      printLog('Update User Body: $body');
  ...
  Future<String> getCheckoutUrl(
    ...
    try {
      ...
      if (response.statusCode == 200 && body is String) {
        ...
        printLog('[WooCommerce.getCheckoutUrl] params: $params');
        printLog(endpoint);
        return endpoint;
      } else {
        var message = body['message'];
  ...
  Future<List<Map<String, dynamic>>> getShippingZones() async {
    try {
      final response =
          await httpGet('$domain/wp-json/custom/v1/shipping-zones'.toUri()!);
      printLog('getShippingZones status: ${response.statusCode}');
      printLog('getShippingZones body: ${response.body}');
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        List? zones;
        if (data is List) {
      ...
      printLog('[getShippingZones] error: $e');
    }
    return [];
  }

  @override
  Future<double> calculateShipping(int zoneId) async {
    try {
      printLog('calculateShipping sent: {"zone_id": $zoneId}');
      final response = await httpPost(
        "$domain/wp-json/custom/v1/calculate-shipping".toUri()!,
        body: jsonEncode({"zone_id": zoneId}),
        headers: {"Content-Type": "application/json"},
      );

      printLog('calculateShipping status: ${response.statusCode}');
      printLog('calculateShipping body: ${response.body}');

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final cost = double.tryParse(data['shipping_cost']?.toString() ?? '0') ?? 0.0;
        printLog('calculateShipping parsed cost: $cost');
        return cost;
      }
    } catch (e) {
      printLog('[calculateShipping] error: $e');
    }
    return 0.0;
  }

  @override
  Future<List<Map<String, dynamic>>> getMetaSlider() async {
    try {
      final response =
          await httpGet('$domain/wp-json/custom/v1/metaslider'.toUri()!);
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data is List) {
          return data
      ...
      printLog('[getMetaSlider] error: $e');
    }
    return [];
  }
}
