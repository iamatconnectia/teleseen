...
class ImageTools {
  ...
  static ImageProvider networkImage(String? url, [kSize size = kSize.medium]) {
    final path = formatImage(url, size) ?? kDefaultImage;
  ...
  static String? getFitnessPlaceholder(String? productName) {
    ...
    if (name.contains('TREADMILL') ||
        name.contains('98550') ||
        name.contains('99150') ||
        name.contains('1017EB') ||
        name.contains('OMA-68')) {
      return kTreadmillImage;
    }
    if ((name.contains('BIKE') && (name.contains('HSM-B') || name.contains('100F') || name.contains('130A'))) ||
        name.contains('KMF-263') ||
        name.contains('KMF-264')) {
      return kExerciseBikeImage;
    }
    if (name.contains('ROWER') ||
        name.contains('7424R') ||
        name.contains('7405C')) {
      return kRowerImage;
    }
    if (name.contains('ELLIPTICAL') ||
        name.contains('ORBITRAC') ||
        name.contains('360E') ||
        name.contains('KMF-217SHP')) {
      return kEllipticalImage;
    }
    return null;
  }
}
